...
import { Text, View, TextInput, Image, StatusBar, Platform, PermissionsAndroid,Alert,TouchableOpacity, SafeAreaView, FlatList, BackHandler } from 'react-native'
...
const Home = observer((props: Props) => {
	...
	const locationEnabler=(c,p)=>{
	  ...
	  const getVehicleType=()=>{
		...
		// method, path, body, header
		db.api.apiCall("get",db.link.getVehicleType,bodyData,header)
		.then((response) => {
	  ...
	  useEffect(() => {
		  if(refresh){
			  if(generalmanager.internet){
				setload(false);setisServerError("A");setrefresh(false);setvchltypeOnce(false) 
				getVehicleType()
			  }else{
				  setrefresh(false);
	...
	const renderCategory=()=>{
		...
		const c = vt.map((e,i,a)=>{
			...
			return(
				<View style={{alignItems:"center",justifyContent:"center",width:65,marginHorizontal:10}}>
				<TouchableOpacity style={styles.Category} onPress={()=>gotoSearch(requestmanager.req?requestmanager.req.type:e)}>
				 <Image source={image} style={styles.CatImage} />
				</TouchableOpacity>
				<Text numberOfLines={1} ellipsizeMode="tail" style={styles.CatText}>{t}</Text>
			</View>
			)
		...
		<SideMenuView style={{ flex: 1 }} drawerName={'customDrawer'}>
			...
			<SafeAreaView style={styles.Container}>
				<StatusBar
					...
					backgroundColor={'#fff'} />
				{showInternerm && <utils.TopMessage msg={"Please Connect internet !"} />}	
				<View style={styles.Header}>
					<TouchableOpacity onPress={gotoProfile} activeOpacity={0.8} style={{flexDirection:"row",alignItems:"center"}}> 
					<utils.vectorIcon.FontAwesome  name="user-circle" color="#0E47A1" size={27} />
				...
				<ScrollView>
					<View style={styles.Body}>
						<LinearGradient colors={['#0e47a1', '#002171']} style={styles.TopCard}>
							<Text style={styles.TopCardTitle}>Transport anywhere, anytime</Text>
							<Text style={styles.TopCardDescription}>Get hassle free goods transport services with </Text>
							<Text style={[styles.TopCardDescription,{fontFamily: 'Inter-Bold' }]}>DeliverIt Mini</Text>
						</LinearGradient>
								...
								<View style={styles.CategoryBox}>
									...
									{(!generalmanager.internet  && !load && !vt)&&(
									 <Text style={{fontSize:12,color:"red"}}>Please connect internet !</Text>
									)}
							    </View>
						...
						<View style={styles.Card}>
							...
							<Image source={require('../../assets/images/delivery.jpg')} style={styles.CardImage} />
							<Text style={styles.CardDescription}>Book a transport vehicle with DeliverIt and transport your goods anywhere you want.</Text>
							<TouchableOpacity style={styles.SearchView}  onPress={()=>gotoSearch(requestmanager.req?requestmanager.req.type:vt?vt[0]:"")}>
								<View style={{ height: 10, width: 10, marginTop: 6, backgroundColor: '#0E47A1', borderRadius: 10 }}></View>
								<Text style={{ width: '90%', height: '100%', color: 'silver', fontFamily: 'Inter-Regular', marginLeft: 5 }}>Enter your dstination</Text>
								<FontAwesome name='search' color='#0E47A1' size={20} />
							</TouchableOpacity>
							<TouchableOpacity style={styles.CardButton}  onPress={()=>gotoSearch(requestmanager.req?requestmanager.req.type:vt?vt[0]:"")}>
								<Text style={{ color: '#fff', fontFamily: 'Inter-Bold' }}>Select Destination</Text>
							</TouchableOpacity>
						</View>
